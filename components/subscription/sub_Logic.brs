    ...
    if purchases.GetChildCount() > 0 
        ...
        for each purchase in allPurchases
            ...
            if utimeExpire > utimeNow and purchase.code="2.0_tiktok_basic" or purchase.code="2.0_tiktok_premium"
                'PackgeSubscraption()
                if purchase.code="2.0_tiktok_basic"
                    ' purchase.cost = "purshased"
                    m.book_now1.text="Purchased"
                    m.book_now1.translation ="[570, 510]"
                    m.book_now1.textColor ="0x16f5f1"
                    m.book_now1.maxWidth = "170"
                    m.book_now1.minWidth = "165"
                    m.book_now1.height = "45"

                else if purchase.code="2.0_tiktok_premium"
                    ' purchase.cost = "purshased"
                    m.book_now2.text="Purchased"
                    m.book_now2.translation="[875, 510]"
                    m.book_now2.textColor ="0x16f5f1"
                    m.book_now2.maxWidth = "170"
                    m.book_now2.minWidth = "165"
                    m.book_now2.height = "45"
                end if
                return
            end if
...
sub OnGetCatalog(event as object)
    ...
    if catalog.GetChildCount() > 0
        ...
        m.activeCatalogItems = []
        for each product in catalog.GetChildren(-1, 0)
            subscriptions.Push(product.name + " " + product.cost.toStr())
            ...
            if product.code = "2.0_tiktok_basic"
                m.basic_cost.text = product.cost
            end if

            if product.code = "2.0_tiktok_premium"
                m.premium_cost.text = product.cost
            end if
...
sub DoSubscriptionOrder(event as object)
    ...
    m.global.channelStore.ObserveField("orderStatus", "OnOrderStatus1")
end sub

sub OnOrderStatus1(event as object)
